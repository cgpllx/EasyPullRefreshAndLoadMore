...
import cc.easyandroid.pullrefresh.util.PtrCLog;
 ...
 */
public class PullRefreshFrameLayout extends ViewGroup {
    ...
    protected RefreshListener mPtrHandler;
    // working parameters
    ...
    public PullRefreshFrameLayout(Context context, AttributeSet attrs, int defStyle) {
        super(context, attrs, defStyle);

        mPtrIndicator = new PtrIndicator();

        TypedArray arr = context.obtainStyledAttributes(attrs, R.styleable.PullRefreshFrameLayout, 0, 0);
        if (arr != null) {

            mHeaderId = arr.getResourceId(R.styleable.PullRefreshFrameLayout_ptr_header, mHeaderId);
            mContainerId = arr.getResourceId(R.styleable.PullRefreshFrameLayout_ptr_content, mContainerId);

            mPtrIndicator.setResistance(
                    arr.getFloat(R.styleable.PullRefreshFrameLayout_ptr_resistance, mPtrIndicator.getResistance()));

            mDurationToClose = arr.getInt(R.styleable.PullRefreshFrameLayout_ptr_duration_to_close, mDurationToClose);
            mDurationToCloseHeader = arr.getInt(R.styleable.PullRefreshFrameLayout_ptr_duration_to_close_header, mDurationToCloseHeader);

            float ratio = mPtrIndicator.getRatioOfHeaderToHeightRefresh();
            ratio = arr.getFloat(R.styleable.PullRefreshFrameLayout_ptr_ratio_of_header_height_to_refresh, ratio);
            mPtrIndicator.setRatioOfHeaderHeightToRefresh(ratio);

            mKeepHeaderWhenRefresh = arr.getBoolean(R.styleable.PullRefreshFrameLayout_ptr_keep_header_when_refresh, mKeepHeaderWhenRefresh);

            mPullToRefresh = arr.getBoolean(R.styleable.PullRefreshFrameLayout_ptr_pull_to_fresh, mPullToRefresh);
            arr.recycle();
        }
    ...
    public void setPtrHandler(RefreshListener ptrHandler) {
